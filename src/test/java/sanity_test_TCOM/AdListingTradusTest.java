...
public class AdListingTradusTest extends BaseClass {
	 ...
	 */
	@Test
	public void verifyAllFiveCategoryIconWithNameOnClickingRestButton() throws Exception {
		...
		String[] categories= {"Transport","Farm","Construction","Spare parts","Material handling"};
	...
	public void verifySelectedCategoryInHomePageisDisplayingAsFilterinListingPage() throws Exception {
		...
		if(result_home!=0) {
			try {
				...
				Assert.assertTrue(result_home>0,"For Truck>>MAN>>tgs>>Netherlands>>1980>>500000>>600000>>44000, search result shows Zero results");
			}
		waitForElement(homePageObj.span_listingPageVerify_xpath);
		waitForElement(adListingObj.select_MakeFilter_xpath);
		Assert.assertTrue(getValue(driver,adListingObj.select_MakeFilter_xpath).equals("man"),"Correct Make name isn't displaying in Listing page Filters");
		Assert.assertTrue(getValue(driver,adListingObj.select_ModelFilter_xpath).equals("tgs"),"Correct Model name isn't displaying in Listing page Filters");
		String temp=getText(adListingObj.div_LocationFilter_xpath).replace("\n",",");
		String[] elements = temp.split(" "); 
		Assert.assertTrue(elements[0].equals("Netherlands"),"Correct Country name isn't displaying in Listing page Filters");
	   ...
	   Assert.assertTrue(temp_filters.contains("results for Transport , Trucks , Year (From): 1980 , Mileage (To): 600000 , Gross Weight (To): 44000 , Model: TGS , Make: MAN , Country: Netherlands , Price (To): 500000 ,"),"All selected filters aren't displaying correctly beside the Ads count ");
	...
	/*
	 * Description: Verify Pagination is displaying in listing page and Verify by default page "1" is aligned as bold
	 */
	@Test
	public void verifyPageinationBehaviour() throws Exception {
		HomeTradusPage homePageObj=new HomeTradusPage(driver);
		explicitWaitFortheElementTobeVisible(driver,homePageObj.element_homePageVerify_xpath);
		int L1_size=homePageObj.icon_L1Categories_xpath.size();
		...
		int size=adListingObj.span_pageNumbers_xpath.size();
		Assert.assertTrue(size==8, "Page counter isn't displaying properly in Listing page");
		Assert.assertTrue(!adListingObj.span_pageNumbers_xpath.get(0).getTagName().equals("a"), "1st page number is acting as Link");
		Assert.assertTrue(adListingObj.span_pageNumbers_xpath.get(0).getCssValue("color").equals("rgba(255, 255, 255, 1)"),"First pagination button isn't highlighted in First Listing page");
		try {
			click(adListingObj.span_pageNumbers_xpath.get(1));
		}catch(Exception E) {
			jsClick(driver,adListingObj.span_pageNumbers_xpath.get(1));
		}
		waitTill(1000);
		waitForElement(homePageObj.span_listingPageVerify_xpath);
		Assert.assertTrue(adListingObj.span_pageNumbers_xpath.get(0).getTagName().equals("a"), "First page number isn't acting as Link when user is in second Listing page");
		Assert.assertTrue(!adListingObj.span_pageNumbers_xpath.get(1).getTagName().equals("a"), "Second page is actiong as Link when user is in second Listing page");
		//System.out.println("for 1st button on 2nd page"+adListingObj.span_pageNumbers_xpath.get(0).getCssValue("color"));
		//System.out.println("for 1st button on 2nd page"+adListingObj.span_pageNumbers_xpath.get(1).getCssValue("color"));
		Assert.assertTrue(adListingObj.span_pageNumbers_xpath.get(1).getCssValue("color").equals("rgba(255, 255, 255, 1)"),"Second pagination button isn't highlighted in Second Listing page");
		try {
			click(homePageObj.logo_tradus_xpath);
		}catch(Exception E) {
			jsClick(driver,homePageObj.logo_tradus_xpath);
	...
	/*
	 * Description: Verify Back to top link along with upward arrow is  Present
	 */
	@Test
	public void verfyBackToTopLinkanditsFunctionality() throws Exception {
		...
		explicitWaitFortheElementTobeVisible(driver,adListingObj.link_BackToTop_xpath);
		try {
			click(adListingObj.link_BackToTop_xpath);
		}catch(Exception E) {
			jsClick(driver,adListingObj.link_BackToTop_xpath);
		}
		waitTill(3000);
	...
	public void verifyAlertAlongwithButtonisDisplaying() throws Exception {
		...
		Assert.assertTrue(verifyElementPresent(adListingObj.div_createAlertText_xpath), "Creat alert section isn't displaying for "+i+"th category Listing");
		Assert.assertTrue(getText(adListingObj.div_createAlertText_xpath).replace("\n", ",").trim().equals("Create alerts for new offers,Receive new offers based on filter options you apply on this page."), "Creat alert section text isn't correct for "+i+"th category Listing");
		try {
		...
		Assert.assertTrue(getText(adListingObj.div_successMessageInCreatingAlert_xpath).trim().equals("Your alert has been created successfully."), "Correct sucess message isn't displaying for "+i+"th category Listing");
		try {
		...
		explicitWaitFortheElementTobeVisible(driver,adListingObj.div_successMessageInCreatingAlert_xpath);
		//System.out.println(getText(adListingObj.div_successMessageInCreatingAlert_xpath).trim());
		Assert.assertTrue(getText(adListingObj.div_successMessageInCreatingAlert_xpath).trim().equals("Your alert has been removed."), "Correct sucess message isn't displaying for "+i+"th category Listing");
		try {
			click(homePageObj.logo_tradus_xpath);
		}catch(Exception E) {
			jsClick(driver,homePageObj.logo_tradus_xpath);
	...
	public void verifyLoginPageDisplayingOnClickingCreateButtonwithOutLogin() throws Exception {
		...
		if(verifyElementPresent(adListingObj.icon_categoryPickerList_xpath))
			try {
				click(adListingObj.icon_L3CategoryIcon_xpath.get(i));
			}catch(Exception E) {
				jsClick(driver,adListingObj.icon_L3CategoryIcon_xpath.get(i));
			}
		waitForElement(adListingObj.div_createAlertText_xpath);
	...
	/*
	 * Description: Verify SEO contains and able to select those
	 */
	@Test
	public void verifySEOLinksanditsNavigation() throws Exception {
		HomeTradusPage homePageObj=new HomeTradusPage(driver);
		explicitWaitFortheElementTobeVisible(driver,homePageObj.element_homePageVerify_xpath);
		waitTill(2000);
		int size=homePageObj.icon_L1Categories_xpath.size();
		...
		waitForElement(homePageObj.span_listingPageVerify_xpath);
		AdListingTradusPage adListingObj=new AdListingTradusPage(driver);
		Assert.assertTrue(verifyElementPresent(adListingObj.div_Make_SEO_Content_xpath), "SEO Contains for Make isn't present for "+i+"th category Listing");
		Assert.assertTrue(verifyElementPresent(adListingObj.div_Country_SEO_Content_xpath), "SEO Contains for Model isn't present for "+i+"th category Listing");
		String country=getText(adListingObj.div_CountryList_SEO_Content_xpath.get(i));
		String make=getText(adListingObj.div_MakeList_SEO_Content_xpath.get(i));
		System.out.println(country);
		System.out.println(make);
		scrollDown(driver);
		waitTill(2000);
		try {
			click(adListingObj.div_CountryList_SEO_Content_xpath.get(i));
		}catch(Exception E) {
		    actionClick(driver,adListingObj.div_CountryList_SEO_Content_xpath.get(i));
		}
		waitTill(1000);
		explicitWaitFortheElementTobeVisible(driver,adListingObj.div_Make_SEO_Content_xpath);
		try {
			click(adListingObj.div_MakeList_SEO_Content_xpath.get(i));
		}catch(Exception E) {
		    jsClick(driver,adListingObj.div_MakeList_SEO_Content_xpath.get(i));
		}
		waitTill(1000);
		explicitWaitFortheElementTobeVisible(driver,adListingObj.select_MakeFilter_xpath);
		System.out.println(getValue(driver,adListingObj.select_MakeFilter_xpath));
		//Assert.assertTrue(getValue(driver,adListingObj.select_MakeFilter_xpath).equals(make),"Correct Make name isn't displaying in Listing page Filters when selected from SEO Links");
		String temp=getText(adListingObj.div_LocationFilter_xpath).replace("\n",",");
		String[] elements = temp.split(" "); 
		System.out.println(elements[0].equals(country));
		//Assert.assertTrue(elements[0].equals(country),"Correct Country name isn't displaying in Listing page Filters when selected from SEO Links");
		try {
			click(homePageObj.logo_tradus_xpath);
		}catch(Exception E) {
			jsClick(driver,homePageObj.logo_tradus_xpath);
