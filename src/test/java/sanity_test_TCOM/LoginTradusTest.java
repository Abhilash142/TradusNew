...
public class LoginTradusTest extends BaseClass {
	...
	/*
	 * Description: Verify Register tab and Social links register option
	 */
	@Test
	public void verifyRegisterTabAndSocialLinks() throws Exception {
		HomeTradusPage homePageObj = new HomeTradusPage(driver);
		LoginTradusPage loginPageObj = new LoginTradusPage(driver);
		explicitWaitFortheElementTobeVisible(driver, homePageObj.link_loginTradus_xpath);
		try {
		...
		waitTill(1000);
		Assert.assertTrue(verifyElementPresent(loginPageObj.link_SignUpwithGoogle_xpath),
				"Sign up with google link isn't present in Sign up page");
		Assert.assertTrue(verifyElementPresent(loginPageObj.link_SignUpwithFb_xpath),
				"Sign up with Facebook link isn't present in Sign up page");
	}
	...
	/*
	 * Description: Verify pop up is displaying on trying to signin to the account with the same buyer and seller email  and 
	 * Verify home page is displaying on clicking the 'Login as buyer' button
	 */
	@Test
	public void verifyPopUpDisplayingWhileLoginInWithSellerAccount() throws Exception {
		HomeTradusPage homePageObj = new HomeTradusPage(driver);
		LoginTradusPage loginPageObj = new LoginTradusPage(driver);
		explicitWaitFortheElementTobeVisible(driver, homePageObj.link_loginTradus_xpath);
		try {
		...
		try {
		 ...
		 Assert.assertTrue(verifyElementPresent(loginPageObj.tradusLoginAsaSeller), "Login As A Seller button isn't present in Buyer Seller model");
		 Assert.assertTrue(verifyElementPresent(loginPageObj.tradusLoginAsaBuyer), "Login As A Buyer button isn't present in Buyer Seller model");
		 try {
				click(loginPageObj.tradusLoginAsaBuyer);
			} catch (Exception E) {
				jsClick(driver, loginPageObj.tradusLoginAsaBuyer);
			}
		 SoftAssert softAssertion=new SoftAssert();
		 ...
		 try {
			 explicitWaitFortheElementTobeVisible(driver,homePageObj.element_homePageVerify_xpath);
				 ...
				 softAssertion.assertTrue(verifyElementPresent(homePageObj.element_homePageVerify_xpath), "Home Page isn't displaying on clicking login in as a Buyer from the buyer seller model");
			 }
		 ...
		 waitTill(1000);
		 explicitWaitFortheElementTobeVisible(driver,homePageObj.link_SignOut_xpath);
		 try {
	...
	/*
	 * Description: Navigation of Social sites link in Registration page
	 */
	@Test
	public void verifySocialSitesNavigationLinkinRegistrationPage() throws Exception {
		HomeTradusPage homePageObj = new HomeTradusPage(driver);
		LoginTradusPage loginPageObj = new LoginTradusPage(driver);
		explicitWaitFortheElementTobeVisible(driver, homePageObj.link_loginTradus_xpath);
		try {
	...
	/*
	 * Description: Navigation of Social sites link in Login page
	 */
	@Test
	public void verifyNavigationOfSocialSitesLinkInLoginPage() throws Exception {
		HomeTradusPage homePageObj = new HomeTradusPage(driver);
		LoginTradusPage loginPageObj = new LoginTradusPage(driver);
		explicitWaitFortheElementTobeVisible(driver, homePageObj.link_loginTradus_xpath);
		try {
		...
		try {
			...
			softAssertion.assertTrue(verifyElementPresent(loginPageObj.input_emailFieldinGooglePage_xpath),
					"Google page didn't displayed on clicking FB link from Login page");
		}
		driver.close();
		driver.switchTo().window(parent_window);
		softAssertion.assertAll();
