...
public class HomeTradusTest extends BaseClass {
		...
		*/
		...
		public void verifySearchFieldFunctionality() throws Exception {
			HomeTradusPage homePageObj=new HomeTradusPage(driver);
			explicitWaitFortheElementTobeVisible(driver,homePageObj.element_homePageVerify_xpath);
			waitForElement(homePageObj.input_searchField_xpath);
		...
		public void verifyLanguageDropdown() throws Exception {
			HomeTradusPage homePageObj=new HomeTradusPage(driver);
			explicitWaitFortheElementTobeVisible(driver,homePageObj.element_homePageVerify_xpath);
			waitForElement(homePageObj.dropdown_languageOptions_xpath);
		...
		public void verifyLoginPageOnClickingLoginLink() throws Exception {
			...
			try {
				 explicitWaitFortheElementTobeVisible(driver,homePageObj.tab_LoginPageVerify_xpath);
					 ...
					 Assert.assertTrue(verifyElementPresent(homePageObj.tab_LoginPageVerify_xpath), "Login link isn't present in Home page");
				 }
		 }
		 ...
		 */
		@Test
		public void verifyTransportFarmConstructionAndMaterialHandlingCategoriesareDisplayingInHomePage() throws Exception {
			HomeTradusPage homePageObj=new HomeTradusPage(driver);
			explicitWaitFortheElementTobeVisible(driver,homePageObj.element_homePageVerify_xpath);
		 ...
		 */
		@Test
		public void verifyAdDetailsDisplayingOnClickingAdImagefromOfferOfTheDay() throws Exception {
			HomeTradusPage homePageObj=new HomeTradusPage(driver);
			explicitWaitFortheElementTobeVisible(driver,homePageObj.element_homePageVerify_xpath);
			for(int i=0;i<homePageObj.icon_L1Categories_xpath.size();i++) {
				try {
			...
			waitTill(2000);
			explicitWaitFortheElementTobeVisible(driver,homePageObj.link_viewOfferinOfferoftheDay_xpath.get(i));
		 ...
		 */
		@Test
		public void verifyAdDetailsDisplayingOnClickingAdTitlefromOfferOfTheDay() throws Exception {
			HomeTradusPage homePageObj=new HomeTradusPage(driver);
			explicitWaitFortheElementTobeVisible(driver,homePageObj.element_homePageVerify_xpath);
			for(int i=0;i<homePageObj.icon_L1Categories_xpath.size();i++) {
				try {
			...
			waitTill(2000);
			explicitWaitFortheElementTobeVisible(driver,homePageObj.title_offerOfTheDay_xpath.get(i));
			String adTitle=homePageObj.title_offerOfTheDay_xpath.get(i).getText().trim();
			//System.out.println(adTitle);
			try {
			...
			try{
				...
				Assert.assertTrue(verifyElementPresent(adDetailsObj.title_AdDetailsVerify_xpath), "ADP isn't displaying for on clicking offer of the day title for "+i+"th category");
			}
			//System.out.println(adDetailsObj.title_AdDetails_xpath.getText().trim());
			//Assert.assertTrue(adTitle.contains(adDetailsObj.title_AdDetails_xpath.getText().trim()), "Same Ad isn't displaying in ADP on click offer of the day");
		    driver.navigate().back();
		...
		public void sellerSectionwithSeeAllLinkanditsNavigation() throws Exception {
			...
			for(int i=0;i<homePageObj.icon_L1Categories_xpath.size();i++) {
				...
				case 0:
				    ...
				    try {
				    	click(homePageObj.link_checkOutourSeller_xpath.get(i));
				    }catch(Exception E) {
		...
		public void verifyPageisRedirectingToSellerDetailsPageonClickingIndivisualSeller() throws Exception {
			...
			for(int i=0;i<homePageObj.icon_L1Categories_xpath.size();i++) {
				...
				case 0:
				    ...
				    title=getAttribute(homePageObj.li_proSellerListforTransport_xpath.get(num),"title").trim();
				    try {
				...
				case 1:
					    ...
					    title=getAttribute(homePageObj.li_proSellerListforFarm_xpath.get(num),"title").trim();
					    try {
				...
				case 2:
					    ...
					    title=getAttribute(homePageObj.li_proSellerListforConstruction_xpath.get(num),"title").trim();
					    try {
				...
				case 3:
					    ...
					    title=getAttribute(homePageObj.li_proSellerListforMaterialhandling_xpath.get(num),"title").trim();
					    try {
		...
		public void verifyCategroyAdsDisplayingOnCategroyinHomePage() throws Exception {
			...
			for(int i=0;i<homePageObj.icon_L1Categories_xpath.size();i++) {
				...
				case 0:
				    ...
				    System.out.println(getText(homePageObj.header_sellerTitle_xpath).trim());
		 ...
		 *  to add , "Top Manufacturers", "Countries" once it got fixed
		 */
		@Test
		public void verifySEOCategoryareDisplaying() throws Exception {
			...
			//System.out.println(newSEOHeader);
		...
		public void verifySubCatgeoriesalongwithSEOLinksareDisplaying() throws Exception {
			HomeTradusPage homePageObj=new HomeTradusPage(driver);
			explicitWaitFortheElementTobeVisible(driver,homePageObj.element_homePageVerify_xpath);
			ArrayList<String> SEOTransportSubCategories=new ArrayList<String>(Arrays.asList("Trucks", "Vans", "Semi-trailers", "Tractor units", "Trailers", "Utility service","Buses / coaches","Other transport"));
			ArrayList<String> SEOFarmSubCategories=new ArrayList<String>(Arrays.asList("Tractors","Hay/Forage","Fertilizer applicators", "Tillage", "Harvesting", "Planting / Sowing", "Agri trailers", "Livestock equipment", "Forestry machinery", "Garden machinery", "Processing and storage", "Potato equipment", "Wine growing equipment", "Irrigation equipment","Special / Other"));
			ArrayList<String> SEOConstructionSubCategories=new ArrayList<String>(Arrays.asList("Excavators", "Loaders", "Attachments", "Telescopic handlers", "Aerial platforms","Other machinery","Road construction equipment", "Cranes", "Dump trucks","Generators", "Crushing equipment", "Dozers", "Compressors", "Drilling equipment","Concrete equipment"));
			ArrayList<String> SEOMHSubCategories=new ArrayList<String>(Arrays.asList("Forklifts","Pallet trucks","Stackers","Reach trucks", "Container handler", "Order pickers","Side loaders", "Rough terrain forklift", "Other warehouse equipment","Tow tractor","Reach stacker","Narrow aisle truck", "4-Way reach trucks"));
		    ...
		    switch(i) {
		    case 0:
		    	if(verifyElementPresent(homePageObj.link_showMoreLinkForTransport_xpath) && getText(homePageObj.link_showMoreLinkForTransport_xpath).contains("Show more")) {
		    		jsClick(driver,homePageObj.link_showMoreLinkForTransport_xpath);
		    	}
		    	waitTill(2000);
		    	...
		    	Collections.sort(SEOTransportSubCategories);
		    	...
		    	break;
		    case 1:
		    	if(verifyElementPresent(homePageObj.link_showMoreLinkForFarm_xpath) && getText(homePageObj.link_showMoreLinkForFarm_xpath).contains("Show more")) {
		    	...
		    	Collections.sort(SEOFarmSubCategories);
		    	Collections.sort(newList);
		    ...
		    case 2:
		    	...
		    	Collections.sort(SEOConstructionSubCategories);
		    	Collections.sort(newList);
		    ...
		    case 3:
		    	...
		    	Collections.sort(SEOMHSubCategories);
		    	Collections.sort(newList);
		      ...
		      case 4:
		    	...
		    	Collections.sort(SEOtopManufacturers);
		    	Collections.sort(newList);
		    	//System.out.println(SEOtopManufacturers);
		    	//System.out.println(newList);
		    	//Assert.assertTrue(SEOtopManufacturers.equals(newList),"All Top Manufactures links aren't displaying properly for Top Manufacturers");
		    	Assert.assertTrue(newList.size()>10,"All Top Manufactures links aren't displaying properly for County List ");
		    	newList.clear();
		    ...
		    case 5:
		    	...
		    	Collections.sort(SEOCountries);
		    	Collections.sort(newList);
		    	//System.out.println(SEOCountries);
		    //	System.out.println(newList);
		    	Assert.assertTrue(newList.size()>10,"All countries links aren't displaying properly for County List ");
		    //	Assert.assertTrue(SEOCountries.equals(newList),"All countries links aren't displaying properly for County List");
		    	newList.clear();
		...
		public void verifyNavigationToAdListingPageOnClickingResultsButtonWithOutApplyingFilters() throws Exception {
			...
			//	System.out.println(resultsInHomePage);
				...
				softAssertion.assertTrue(result_listing>=result_home-600 || result_listing<=result_home+600, "For "+i+" category Ad count from Home page doesn't match with Listing");
		...
		public void verifyListingPageAfterApplyingAllMaterialHandlingCategoryFilters() throws Exception {
			...
			for(int i=0;i<filters.length;i++) {
				...
				//	System.out.println(countries.size());
					...
					waitTill(3000);
					/*
					 * System.out.println(make.getOptions().size()); for(int
					 * z=0;z<make.getOptions().size();z++) {
					 * System.out.println(make.getOptions().get(z).getText()); }
					 */
				...
				if(i==1) {
					int size=model.getOptions().size();
					/*
					 * System.out.println(size); for(int z=0;z<model.getOptions().size();z++) {
					 * System.out.println(model.getOptions().get(z).getText()); }
					 */
					if(size>1) {
						int model_Num=Integer.parseInt(generateRandomNumber(0, (model.getOptions()).size()-1));
						model.selectByIndex(model_Num);
			...
			if(result_home==0) {
				...
				try {
					 explicitWaitFortheElementTobeVisible(driver,homePageObj.span_listingPageVerify_xpath);
						 ...
						 Assert.assertTrue(verifyElementPresent(homePageObj.span_listingPageVerify_xpath), "Listing page isn't displayed when clicked on results button for Material Handling Category with other filters");
					 }
				String resultsInListingPage=getText(adDListingObj.text_adCountWithFiltersinListingPage_xpath).replace("cross","").trim();
				String[] elements = resultsInListingPage.split(" "); 
				int result_listing=Integer.parseInt(elements[0].replaceAll("[^0-9]", ""));
